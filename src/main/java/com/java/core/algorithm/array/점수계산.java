...
public class 점수계산 {
     ...
     *
     * 1번 문제가 맞는 경우에는 1점으로 계산한다. 앞의 문제에 대해서는 답을 틀리다가 답이 맞는
     * 처음 문제는 1점으로 계산한다.
     *
    ...
    public static void main(String[] args) {
        ...
        int icjiSolution = icjiSolution(count, score);
        System.out.println("icjiSolution = " + icjiSolution);

        int solution = solution(count, score);
        System.out.println("solution = " + solution);
    }

    private static int icjiSolution(int count, int[] score) {
        int answer = 0;
    ...
    private static int solution(int count, int[] score) {
        int answer = 0;
        int cnt = 0;
